//------------------------------------------------------------------------------
// name: mosaic-synth-mic-norm.ck (v1.3)
...
//        > chuck mosaic-synth-mic-norm.ck:INPUT:MINMAX
//
// date: Spring 2023
...
if( me.args() > 0 )
{
    ...
    if( me.args() > 1 )
    {
        me.arg(1) => MINMAX_FILE;
    }
}
else
{
    // print usage
    <<< "usage: chuck mosaic-synth-mic.ck:INPUT:MINMAX", "" >>>;
...
float dimMin[NUM_DIMENSIONS]; // array reference for mins
float dimMax[NUM_DIMENSIONS]; // array reference for maxs
// load min/max for each dimension
if( loadMinMaxFile( MINMAX_FILE ) )
{
    // set flag
    true => DO_NORMALIZE;
}
...
fun void normalize( float v[] )
{
    ...
    if( v.size() != dimMin.size() )
    {
        <<< "normalize(): dimension mismatch -- expecting", v.size(), "but min/max has", dimMin.size() >>>;
        return;
    }
    ...
    for( int i; i < v.size(); i++ )
    {
        ...
        Math.map( v[i], dimMin[i], dimMax[i], 0, 1 ) => v[i];
        // cherr <= v[i] <= " ";
    }
    // cherr <= IO.newline();
}
...
// function: load data file
//------------------------------------------------------------------------------
fun int loadMinMaxFile( string filepath )
{
    ...
    while( fio.more() )
    {
        ...
        if( line != "" )
        {
            ...
            {
                // min
                tokenizer.next() => Std.atof => dimMin[d];
                // more?
                if( tokenizer.more() )
                {
                    // max
                    tokenizer.next() => Std.atof => dimMax[d];
                }
            }
