//------------------------------------------------------------------------------
// name: mosaic-synth-doh.ck (v1.3)
// desc: basic structure for a feature-based synthesizer
//       here we generate our audio input to be mosaic'ed
//       with a little help from DOH
//
// version: need chuck version 1.4.2.1 or higher
// sorting: part of ChAI (ChucK for AI)
//
// USAGE: run with INPUT model file
//        > chuck mosaic-synth-doh.ck:INPUT
//
...
for( int i; i < NUM_VOICES; i++ )
{
    ...
    envs[i].set( EXTRACT_TIME/2, EXTRACT_TIME/256, 1, EXTRACT_TIME/2 );
}
...
fun void synthesize( int uid )
{
    ...
    envelope.keyOn();
    // wait
    (EXTRACT_TIME*1.7)-envelope.releaseTime() => now;
    // start the release
    envelope.keyOff();
    // wait
